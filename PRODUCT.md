...
|--------|------|
| 「睡觉」/「挂单」 | 生成睡前限价挂单计划 |
| 「仓位」 | 显示当前持仓概览（含现金余额、总资产） |
| 「报告」/「盯盘」 | 生成完整市场报告 |
| 「评分 NVDA」 | 单只标的评分详情 |
| 「查 NVDA」 | 单只标的技术分析 |
| 「加仓MRVL和MU」 | **Portfolio-aware AI 建议**（含持仓、盈亏、TA、仓位建议） |
| 自由提问 | 基于市场数据 + 持仓上下文的 AI 回答 |

#### Portfolio-Aware Chat（新）

当用户在自由对话中提到具体持仓标的时，系统自动注入：
- 该标的当前仓位（数量、成本、盈亏%）
- 近期交易活动和冷却期状态
- 实时技术指标（RSI、MA20、趋势方向）
- 账户现金余额和总资产

DeepSeek 在此上下文下给出**具体仓位大小建议**，而非泛泛的市场评论。
...
| `/api/portfolio` | GET | 仓位概览（含现金、总资产） |
| `/api/portfolio/cash` | POST | 现金调整（入金/出金） |
| `/api/chat` | POST | AI 对话（自动注入持仓上下文） |
| `/api/health` | GET | 健康检查 |
...
- 用 AI 辅助理解复杂的多标的持仓状态（提问时自动注入仓位上下文）
- 快捷入金/出金管理，保持现金余额准确
